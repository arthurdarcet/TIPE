...
let rec lire_nouvelle_ligne_bin sép f =				(* Lit la prochaine ligne du in_channel qui est n'est pas un commentaire, les différentes lignes sont séparées par le caractère sép *)
	let c =  input_char f in
	if c = '#' then									(* Si la ligne en cours commence par un #, c'est un commentaire : on la saute *)
		begin 										(* on lit caractère par caractère jusqu'au prochain séparateur, puis on recommence, pour gérer plusiseurs comentaires de suite *)
			while input_char f <> sép do () done;
...
let image_of_P3 s =									(* Lit l'image ppm de type P3 (codage ASCII) et renvoi l' "image" correspondante *)
	...
	let dim = lire_nouvelle_ligne fichier in		(* La deuxième ligne est de la forme (nombre de pixels en largeur) espace (nombre de pixels en hauteur). lire_dim la décode *)
