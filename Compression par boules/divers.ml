...
let ts_debut = ref (gettimeofday ());;						(* Référence car la valeur doit etre remise à jour dans interface.ml *)

let sous_windows () =										(* Indique si le programme est exécuté sous Windows *)
	try ignore (Unix.nice 1); false	with invalid_arg -> true
;;

let afficher_heure () =										(*  Affiche  le nombre de minutes et de secondes écoulées depuis le début *)
	let ts = gettimeofday () in
	let transf n = if n < 10 then "0"^(string_of_int n) else (string_of_int n) in
...
let afficher s lvl =										(* Affiche le temps écoulé, et un message au niveau lvl : 2*lvl espaces sont placés avant le message *)
	print_endline ((afficher_heure ())^(String.make (2*lvl) ' ')^s);
;;
...
let list_of_tbl table =										(* Transforme le tableau contenant les poids de chaque couleurs en une liste de lettre, enregistrement {couleur, poids} *)
	...
	for j=0 to 255 do
	for k=0 to 255 do 
		if table.(i).(j).(k) <> 0 then liste := {couleur={rouge=i;vert=j;bleu=k} ; poids=table.(i).(j).(k)}::(!liste);
	done;
	done;
	done;
	!liste
...
let copie_queue queue =										(* Fonction de copie de queue, recursivité terminale (contrairement à Queue.copy) *)
	...
	let rec aux () =
		...
		aux ();
	in
	try aux () with Queue.Empty -> q1,q2
;;
