...
;;
let compression cin cout taux_compress dist_coul =				(*  cin et cout sont des string représentant le chemin d'accès au fichier en entrée (cin) et en sortie (cout) *)
	Divers.ts_debut := Unix.gettimeofday ();
	...
	let bin = coder_image img taux_compress dist_coul in		(* Encode l''image' et rend la liste de booléens à inscire dans le fichier de sortie *)
...
let compute ch_in ch_out taux_entry dist_entry () =
	let txt_in = Entry.get ch_in and txt_out = Entry.get ch_out and taux = Entry.get taux_entry and dist = Entry.get dist_entry in
	match String.sub txt_in (String.length txt_in - 3) 3 with
		| "cxa" -> decompression txt_in txt_out;
		| "ppm" -> compression txt_in txt_out ((float_of_string taux)/.100.) (int_of_string dist);
...
let info ch_in () =
	...
	let bin_file = bin_queue_of_cxa f in					(* On récupère d'un coup toute la queue de booléens correspondant au fichier *)
	let (col,lignes) = décode_dim bin_file in				(* 24 premiers bits codent les dimensions *)
	let max = int_of_bin (queue_obtenir bin_file 8) in		(* 8 suivants codent la longueur maximale des transcriptions *)
	let case = int_of_bin (queue_obtenir bin_file 12) in	(* 12 suivants codent le coté des cases utilisées pour coder l'image *)
	let taux = (float_of_int (int_of_bin (queue_obtenir bin_file 10)))/.1000. in
	let dist = int_of_bin (queue_obtenir bin_file 8) in
...
let init () =
	...
	let frame_dial = Frame.create root in

	let txtvar = Textvariable.create () and t2 = Textvariable.create () and t3 = Textvariable.create () in
	...
	let dial = Label.create frame_dial in

	let boutton = Button.create root ~text:"Compresser / Décompresser" ~command: (compute ch_in ch_out taux dist) in
	...
	pack [frame_dial] ~side:`Top ~expand:true ~fill:`Both ;
	pack [dial];
